...
begin
  ...
  try
    ...
    //load the image data
    GetMem(image, imgsize);
    BlockRead(tgafile, image^, imgsize, bytesread);
    ...
    begin
      front := Pointer(Integer(image) + I*channels);
      back := Pointer(Integer(image) + (I*channels) + 2);
      temp := Front^;
      front^ := Back^;
      back^ := Temp;
    end;
    ...
    CreateTexture(width, height, channels, params[0].int, texid, image);
